...
{
	list <simpleInstance *>::iterator iter, itertemp_a, itertemp_b;
	simpleInstance *anInst, *insttemp_a, *insttemp_b;
	double temp_X, temp_Y;
	double curr_cost, trial_cost, delta_cost, simpp, simann, leg, impro;
	double r, p;
	int selected_cell_a, selected_cell_b;
	int temp_row;
	int rowNo, i;
	int j, k, l;
	int num_cell = 0;
	...
	curr_cost = THPWL();
	simpp = curr_cost;
	printf("SimplePlacer cost = %f\n", curr_cost);

	int num_iter = num_cell * it;

	while (T > T_min)
	{	
		for (j = 0; j < num_iter; j++)
        	{
			selected_cell_a = rand() % num_cell;
			selected_cell_b = rand() % num_cell;
			itertemp_a = itertemp_b = mList->begin();

			for (k = 0; k < selected_cell_a; k++)
			{		
				itertemp_a++;
			}
			for (k = 0; k < selected_cell_b; k++)
			{
				itertemp_b++;
			}

			insttemp_a = (simpleInstance *)(*itertemp_a);
			insttemp_b = (simpleInstance *)(*itertemp_b);

			temp_X = insttemp_a->TL.x;
			temp_Y = insttemp_a->TL.y;
			temp_row = insttemp_a->rowNo;

			insttemp_a->TL.x = insttemp_b->TL.x;
			insttemp_a->TL.y = insttemp_b->TL.y;
			insttemp_a->rowNo = insttemp_b->rowNo;

			insttemp_b->TL.x = temp_X;
			insttemp_b->TL.y = temp_Y;
			insttemp_b->rowNo = temp_row;
	...
	printf("Simulated Annealing cost = %f\n", curr_cost);
	simann = curr_cost;
	impro = round(((simpp - simann) / simpp) * 100);
	printf("The improvement after Simulated Annealing is almost equal to : %d%\n", (int)impro);

	// Checking for non-interference of cells with each other

	int legal[3228]={0};

	for (l = 0; l <= rowNumber; l++)
	{
		int loc = 0;
	    	int cell_loc;
		double row_width = 0;
			...
			loc = 0;
		 	for (iter = mList->begin(); iter != mList->end(); iter++)
		 	{
		   		...
		   		if (anInst->rowNo == l && legal[loc] == 0 && anInst->TL.x < min )
		     		{
		       			insttemp_a = anInst;
		       			flag = true;
		       			min = anInst->TL.x;
		 	...
		 	if(flag == true)
		   	{
		     		insttemp_a->TL.x = row_width;
		     		row_width += insttemp_a->Width;
		     		legal[cell_loc] = 1;
		   	}
	     	}

	  }
	  printf("Cost after legalization = %f\n", THPWL());
	  leg = THPWL();
	  impro = round(((simpp - leg) / simpp) * 100);
	  printf("The improvement after legalization is almost equal to : %d%\n", (int)impro);
	  return 1;
}
